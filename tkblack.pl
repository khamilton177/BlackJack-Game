...
$screen=MainWindow->new(-bg=>"green", -height=>"6i", -width=>"8i", -title=>"Black Jack");
$screen->packPropagate(0);
$topMenuBar=$screen->Frame(-width=>"8i", -height=>"6m")->pack(-side=>"top");
$bottomMenuBar=$screen->Frame(-width=>"8i", -height=>"6m")->pack(-side=>"bottom");

###Create widgets inside top frame.
$topMenuBar->Menubutton( -text =>"Game")->place(-anchor=>"nw");

$hit=$screen->Button( -text=>"Hit Me", -command=>sub{HitMe})->pack(-padx=>"2m", -side=>"left");
$hit->configure(-width=>30);
$stay=$screen->Button(-text=>"Stay")->pack(-padx=>"2m", -side=>"left");
