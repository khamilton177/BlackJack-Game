...
$IsValid=0;
	...
	### What's response.
		if ($response eq 'q'or $response =~ m/N/i){
			...
			print "\n";
			print "GREAT!!!  How many players?\n";
			print "No more than 5 people may play at one time.  Please enter number of players?\n";
			...
			until ($IsValid){
				...
				###Are there valid players.
				if ($players eq 'q'or $players=~ m/[0-5]/){ 
					$IsValid=1;
				}
				else{
					print "Please enter up to 5 players as a single integer or 'q' to quit\a\n";
				}
			}
		}
